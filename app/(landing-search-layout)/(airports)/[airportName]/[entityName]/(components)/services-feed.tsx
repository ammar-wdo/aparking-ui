...
{services.map((service :Service &{parkingDays?:number,totalPrice?:number,entity:{entityName:string,slug:string,airport:{name:string,slug:string}},totalReviews:number})=><ListCard key={service.id} show service={service} />)}
      </div>
    </div>
  )
}
