    ...
    <div className="mt-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3  xl:grid-cols-4 gap-8">
        {blogs.map((blog) => (
          <Card
            key={blog.id}
            className="flex-col flex border-none overflow-hidden rounded-sm"
          >
            <CardHeader className="p-0">
              <div className="relative w-full aspect-video ">
                <Badge className="absolute z-10 text-[10px] rounded-lg font-normal bg-site capitalize cursor-default hover:bg-site top-3 left-3">
                  {blog.category.label}
                </Badge>
                <Image
                  alt="blog-image"
